  ...
  return (
    <div className="flex flex-col w-full items-center gap-4 gradient-background py-12 px-5">
      <div className="text-center px-2 md:px-0">
        <h1 className="text-3xl md:text-6xl md:leading-relaxed text-primary-100 mb-8 md:mb-8">Diagnostics à la demande</h1>
      </div>
      <div className="max-w-[50rem] px-6 flex flex-col-reverse md:flex-row items-center gap-10 md:gap-12">
        ...
        <p className="text-base md:text-lg text-center md:text-start leading-snug text-primary-100 max-w-[20rem] md:max-w-[23rem]">Services professionnels de diagnostics automobiles et de programmation de clés à votre porte.</p>
      </div>
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3 md:gap-12 mt-6 md:mt-16 mb-10">
        {tabs.map(({ id, title, p, svg }) => (
          <div key={id} className="flex flex-col max-w-[26rem] items-center text-primary-100 backdrop-filter backdrop-blur-3xl backdrop-sepia-[55%] px-4 py-6 mx-2  rounded-xl shadow-xl">
